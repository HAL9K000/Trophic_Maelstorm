...
if [$INIT ne 2]; then
    ...
    bash -c "cd .. ; g++-12 -DSPB=${SPB} -DINIT=${INIT} -DMV_INVARIANCE=${MV_INVARIANCE} -DBARRACUDA -c order_${SPB}stoc_unity_rietkerk.cpp -fopenmp -o -std=c++20 -I/usr/local/cuda/include"
else
    bash -c "cd .. ; g++-12 -DSPB=${SPB} -DINIT=${INIT} -DMV_INVARIANCE=${MV_INVARIANCE} -DBARRACUDA -c order_${SPB}stoc_burnin_rietkerk.cpp -fopenmp -o -std=c++20 -I/usr/local/cuda/include"
fi
echo "Created object file for order_${SPB}stoc ... script"
# Next, compile the main cpp script
bash -c "g++-12 -DSPB=${SPB} -DINIT=${INIT} -DMV_INVARIANCE=${MV_INVARIANCE} -DBARRACUDA -c rietkerk_bjork_FKE.cpp -fopenmp -std=c++20"
