...
if [ $# -lt 2 ]; then
    echo "Usage: $0 <Path/to/init_file.txt> [Optional: <SpB> <Init_Types> <Mv_Invariance>]"
    echo " where SpB is the number of biotic species (default is 3), 
            Init_Types is type of frame initialisation (default is 1) and
            Mv_Invariance is the type of movement invariance (default is 0 = Distance Inv, 1 = Velocity Inv)."
...
start_screen(){
    ...
    if ! screen -list | grep -q "${screen_names[$screen_index]}"; then
        ...
        if [ $init -ne 2 ]; then
            screen -dmS ${screen_names[$screen_index]} bash -c "cd .. ; g++-12 -DSPB=${spb} -DINIT=${init} -DMV_INVARIANCE=${mvinv} rietkerk_bjork_FKE.cpp order_${spb}stoc_unity_rietkerk.cpp -fopenmp -o ${screen_names[$screen_index]}_${prefix}.out -std=c++23; ./${screen_names[$screen_index]}_${prefix}.out $p1 $p2 $p3 $p4 $p5 $p6 $p7 $p8 $p9 $p10 $p11 $p12 &> stderr_${screen_names[$screen_index]}.txt; cd $curr_dir"
        else
        ...
        screen_index=$(start_screen $file_path $spb $screen_index $i $init $mvinv)
    fi
    echo "$screen_index"
}
...
for ((i=0; i<num_screens; i++)); do
    screen_idx=$(start_screen $1 $2 $screen_idx $i $3 $4)
