...
echo " Usage: $0 <path/to/init_file.txt> <job_name> <SPB> [Optional <init =1> <n> <k>], where n is --cpus-per-task and k is the number of jobs in the array."
echo " $0 is FOR LOCUST_WIND condition."
...
read -r p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13 p14 p15  <<< \$(sed -n "\$((SLURM_ARRAY_TASK_ID+2))p" \$init_file)
CPUS_PER_TASK=\$p7
...
if [ $3 -eq 1 ]; then
    ...
    p14=\$(echo \$p14 | tr -d '[:space:]')
    prefix=\$p12
fi
if [ $init -eq 2 ]; then
    # Check if p15 is not empty
    if [ -z "\$p15" ]; then
        p14=$(echo \$p14 | tr -d '[:space:]')
    else
        p15=$(echo \$p15 | tr -d '[:space:]')
    fi
fi

echo \$p1 \$p2 \$p3 \$p4 \$p5 \$p6 \$p7 \$p8 \$p9 \$p10 \$p11 \$p12 \$p13 \$p14 \$p15
...
if [ $init -ne 2 ]; then
    g++ -DSPB=${3} -DINIT=${init} -DLOCUST_WIND rietkerk_bjork_basic.cpp locust_order_${3}stoc_unity_rietkerk.cpp -fopenmp -o ama_${2}_\${SLURM_ARRAY_TASK_ID}.out -std=c++23
else
    g++ -DSPB=${3} -DINIT=${init} -DLOCUST_WIND rietkerk_bjork_basic.cpp locust_order_${3}stoc_burnin_rietkerk.cpp -fopenmp -o ama_${2}_\${SLURM_ARRAY_TASK_ID}.out -std=c++23
fi

# Run the compiled program with the input parameters
./ama_${2}_\${SLURM_ARRAY_TASK_ID}.out \$p1 \$p2 \$p3 \$p4 \$p5 \$p6 \$p7 \$p8 \$p9 \$p10 \$p11 \$p12 \$p13 \$p14 \$p15 &> std_${2}_\${SLURM_ARRAY_TASK_ID}.txt
