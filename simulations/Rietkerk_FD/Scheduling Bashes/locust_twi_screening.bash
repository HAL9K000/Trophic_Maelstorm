...
start_screen(){
    ...
    read -r p1 p2 p3 p4 p5 p6 p7 p8 p9 p10 p11 p12 p13 p14 p15 <<< $(sed -n "$((i+2))p" $1)
    ...
    if ! screen -list | grep -q "${screen_names[$screen_index]}"; then
        ...
        if [ $init -ne 2 ]; then
            screen -dmS ${screen_names[$screen_index]} bash -c "cd .. ; g++-14 -DSPB=${spb} -DINIT=${init} -DLOCUST_WIND rietkerk_bjork_basic.cpp locust_order_${spb}stoc_unity_rietkerk.cpp -fopenmp -o ${screen_names[$screen_index]}_${prefix}.out -std=c++23; ./${screen_names[$screen_index]}_${prefix}.out $p1 $p2 $p3 $p4 $p5 $p6 $p7 $p8 $p9 $p10 $p11 $p12 $p13 $p14 &> stderr_${screen_names[$screen_index]}.txt; cd $curr_dir"
        else
            screen -dmS ${screen_names[$screen_index]} bash -c "cd .. ; g++-14 -DSPB=${spb} -DINIT=${init} -DLOCUST_WIND rietkerk_bjork_basic.cpp locust_order_${spb}stoc_burnin_rietkerk.cpp -fopenmp -o ${screen_names[$screen_index]}_${prefix}.out -std=c++23; ./${screen_names[$screen_index]}_${prefix}.out $p1 $p2 $p3 $p4 $p5 $p6 $p7 $p8 $p9 $p10 $p11 $p12 $p13 $p14 $p15 &> stderr_${screen_names[$screen_index]}.txt; cd $curr_dir"
        fi
